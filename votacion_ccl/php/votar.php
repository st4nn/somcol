<?php
   ...
   if ($fecha > $fechaIni)
   {
      if ($fecha < $fechaFin)
      {
        $sql = "SELECT * FROM ccl_votos WHERE Cedula = '$Cedula' AND Anio = '$Anio';";
        $result = $link->query($sql);

        if ($result->num_rows > 0){
          echo "Tu voto ya estaba registrado, solo puedes votar una vez, gracias por participar";
        } else{
           $sql = "INSERT INTO ccl_votos (idCandidato, Cedula, Anio) VALUES ('$Candidato', '$Cedula', '$Anio');";
           $result = $link->query($sql);

           if ( $link->affected_rows > 0){
              echo "Tu voto ha sido registrado, gracias por participar";
              $sql = "SELECT COUNT(*) AS Votos FROM ccl_votos WHERE idCandidato = '$Candidato';";
              $result = $link->query($sql);
              $fila =  $result->fetch_array(MYSQLI_ASSOC);

              $sql = "UPDATE ccl_AEC_Candidatos SET Votos = " . $fila['Votos'] . " WHERE id = '" . $Candidato . "';";
              $result = $link->query($sql);
           } else
           {
